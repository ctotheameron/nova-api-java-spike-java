...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { NovaApiJavaSpikeInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw NovaApiJavaSpikeInvalidDataException("`$name` is not set")
            is JsonNull -> throw NovaApiJavaSpikeInvalidDataException("`$name` is null")
            else -> throw NovaApiJavaSpikeInvalidDataException("`$name` is invalid, received $this")
        }
